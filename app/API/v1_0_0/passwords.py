        ...
        if value:
            ...
            if len(value.value) > 64 or len(value.value) < 16:
                ...
                password_encrypted = bcrypt.hashpw(
                    ...
                    app.config['SECRET_KEY'].encode('utf8')
                ).decode('utf-8')

            if Passwords.query.filter(
                Passwords.value == password_encrypted and
                Passwords.user_id == user.value
            ).first():
                return json_http_response(
                    status=400,
                    given_message=_(
                        "This password was previously used for the user"
                        " «%(login)s». Choose another password.",
